...
var dir = path.join(__dirname, 'secp256k1')
...
if (process.argv.indexOf('--print-lib') > -1) {
  switch (os.platform()) {
    case 'darwin':
      console.log('../lib/libsecp256k1-' + arch + '.dylib')
      break
    case 'openbsd':
    case 'freebsd':
    case 'linux':
      console.log(path.join(__dirname, 'lib/libsecp256k1-' + arch + '.so'))
      break
    case 'win32':
      console.log('../secp256k1/Build/ReleaseDLL/' + warch + '/libsecp256k1.lib')
      break
    default:
      process.exit(1)
  }
...
function buildWindows () {
  var res = path.join(__dirname, 'lib/libsecp256k1-' + arch + '.dll')
  ...
  spawn('.\\msvc-scripts\\process.bat', [], { cwd: dir, stdio: 'inherit' }, function (err) {
    ...
    spawn(msbuild, args, { cwd: dir, stdio: 'inherit' }, function (err) {
      ...
      var dll = path.join(dir, 'Build/ReleaseDLL/' + warch + '/libsecp256k1.dll')
...
function buildUnix (ext, cb) {
  var res = path.join(__dirname, 'lib/libsecp256k1-' + arch + '.' + ext)
    ...
    spawn('./configure', ['--prefix=' + tmp], { cwd: dir, stdio: 'inherit' }, function (err) {
      ...
      spawn('make', ['clean'], { cwd: dir, stdio: 'inherit' }, function (err) {
        ...
        spawn('make', ['install'], { cwd: dir, stdio: 'inherit' }, function (err) {
          ...
          var la = ini.decode(fs.readFileSync(path.join(tmp, 'lib/libsecp256k1.la')).toString())
