...
function buildWindows () {
  var lib = path.join(__dirname, 'lib/libsecp256k1-' + arch + '.dll')
  var dst = path.join(build, 'libsecp256k1.dll')
...
function buildUnix () {
  var lib = fs.realpathSync(path.join(__dirname, 'lib/libsecp256k1-' + arch + '.so'))
...
function buildDarwin () {
  var lib = path.join(__dirname, 'lib/libsecp256k1-' + arch + '.dylib')
  var dst = path.join(build, 'libsecp256k1.dylib')
  if (fs.existsSync(dst)) return
  copy(lib, dst, function (err) {
    if (err) throw err
